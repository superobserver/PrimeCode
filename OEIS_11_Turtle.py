...
# Function to analyze blocks of 1000 elements and create turtle animations
def analyze_blocks(lst, block_size=1000):
    """Analyze the list in blocks of block_size elements for marks and zeroes, and animate changes."""
    ...
    # Turtle animations for each metric
    def setup_turtle(title):
        """Set up a turtle screen and turtle for plotting."""
        screen = turtle.Screen()
        screen.title(title)
        t = turtle.Turtle()
        t.speed(8)  # Moderate speed for animation
        t.pensize(2)
        return screen, t
    
    def draw_axes(t, x_max, y_min, y_max, x_label, y_label):
        """Draw axes for the turtle plot."""
        t.penup()
        t.goto(-300, -200)  # Origin
        t.pendown()
        t.goto(-300, 200)   # Y-axis
        t.goto(-300, -200)
        t.goto(300, -200)   # X-axis
        t.penup()
        t.goto(-320, 220)
        t.write(y_label, font=("Arial", 12, "normal"))
        t.goto(320, -220)
        t.write(x_label, font=("Arial", 12, "normal"))
        # Draw x-axis labels (block pairs)
        for i, label in enumerate(block_pairs):
            t.goto(-300 + (i * 600 / x_max), -220)
            t.write(label, font=("Arial", 8, "normal"))
    
    def scale_y(value, y_min, y_max):
        """Scale y-value to fit turtle coordinates."""
        if y_max == y_min:
            return 0
        return -200 + (400 * (value - y_min) / (y_max - y_min))
    
    def animate_plot(screen, t, x_values, y_values, title, y_label):
        """Animate a line plot for given data."""
        draw_axes(t, len(x_values), min(y_values), max(y_values), "Block Pair", y_label)
        t.penup()
        t.goto(-300, scale_y(y_values[0], min(y_values), max(y_values)))
        t.pendown()
        for i in range(len(x_values)):
            x = -300 + (i * 600 / len(x_values))
            y = scale_y(y_values[i], min(y_values), max(y_values))
            t.goto(x, y)
            screen.update()
    ...
    metrics = [
        (marks_increases, "Marks Increase Animation", "Marks Increase"),
        (marks_percent_changes, "Marks % Change Animation", "Marks % Change"),
        (zeroes_changes, "Zeroes Change Animation", "Zeroes Change"),
        (zeroes_percent_changes, "Zeroes % Change Animation", "Zeroes % Change")
    ]
    
    for y_values, title, y_label in metrics:
        screen, t = setup_turtle(title)
        animate_plot(screen, t, block_pairs, y_values, title, y_label)
        screen.mainloop()  # Wait for user to close the window before proceeding
