...
# Initialize shared arrays using Manager
array_size = int(limit + 10)
manager = multiprocessing.Manager()
A201804_shared = manager.list([0] * array_size)  # Shared list for A201804
marks_per_operator_shared = [manager.list([0] * array_size) for _ in range(12)]  # Shared lists for each operator
...
def drLD(x, l, m, z, o, A201804_shared, marks_shared, limit):
    y = 90 * (x * x) - l * x + m
    if 0 <= y < len(A201804_shared):
        A201804_shared[y] += 1
        marks_shared[y] += 1
    p = z + (90 * (x - 1))
    q = o + (90 * (x - 1))
    for n in range(1, int(((limit - y) / p) + 1)):
        idx = int(y + p * n)
        if 0 <= idx < len(A201804_shared):
            A201804_shared[idx] += 1
            marks_shared[idx] += 1
...
def process_operators(operator_indices, A201804_shared, marks_per_operator_shared, limit, new_limit, operator_params):
    for x in range(1, new_limit):
        for op_idx in operator_indices:
            ...
            drLD(x, l, m, z, o, A201804_shared, marks_per_operator_shared[op_idx], limit)
...
def main():
    ...
    with multiprocessing.Pool(processes=num_cores) as pool:
        pool.starmap(
            process_operators,
            [(task, A201804_shared, marks_per_operator_shared, limit, new_limit, operator_params) for task in process_tasks]
        )
    
    # Convert shared lists to numpy arrays for final processing
    A201804 = numpy.array(A201804_shared)[:-10]
    marks_per_operator = [numpy.array(marks_per_operator_shared[i])[:-10] for i in range(12)]
