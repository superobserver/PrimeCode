...
def is_composite(n, operator, n_max):
    a, l, m, p, q = operator
    ...
    for x in range(1, x_max):
        ...
        p_x = p + 90 * (x - 1)
        q_x = q + 90 * (x - 1)
        for i in range(1, (n_max - base_n) // p_x + 1):
            if base_n + i * p_x == n:
                return True
        for i in range(1, (n_max - base_n) // q_x + 1):
            if base_n + i * q_x == n:
                return True
    return False

def compute_chunk(args):
    chunk_start, chunk_end, s, operators_dict, coprime_24 = args
    zeta_chunk = {k: 0 for k in coprime_24}
    holes_chunk = {k: 0 for k in coprime_24}
    for n in range(chunk_start, chunk_end + 1):
        for k in coprime_24:
            is_comp = any(is_composite(n, op, chunk_end) for op in operators_dict.get(k, []))
            if not is_comp:
...
if __name__ == '__main__':
    print("Computing Hole Counts and 24-Path Zeta with Amplitude Lists (Streaming)")
    n_max = 23900001
    s = 0.5 + 14.134725j
    ...
    # Define chunks
    chunk_size = (n_max + 1) // 25  # ~956,000 per chunk
    ...
    with Pool(processes=num_cores) as pool:
        chunk_args = [(start, end, s, operators, coprime_24) for start, end in chunks]
        chunk_results = pool.map(compute_chunk, chunk_args)

    # Aggregate partial zetas and holes
    total_zetas = {k: 0 for k in coprime_24}
    total_holes_chunked = {k: 0 for k in coprime_24}
    for zetas, holes in chunk_results:
        for k, zeta in zetas.items():
            total_zetas[k] += zeta
        for k, count in holes.items():
            total_holes_chunked[k] += count

    # Compute final zeta sum
    total_zeta = sum(1 / total_zetas[k] if total_zetas[k] != 0 else 0 for k in coprime_24)
    normalized_zeta = total_zeta * (4 / 15)

    # Output hole counts for verification
    total_holes = sum(total_holes_chunked.values())
    expected_primes = math.floor(2.15e9 / (math.log(2.15e9) - 1))
    print(f"\nTotal Holes (Chunked, 24 classes): {total_holes}")
    print(f"Expected Primes (Li(2.15B)): {expected_primes}")
    print(f"Difference: {total_holes - expected_primes}")
    for k in coprime_24:
        print(f"k = {k}: {total_holes_chunked[k]} holes")
