...
def sieve_subsequence_chunk(n_start, n_end, operators, k, x_max_factor=250):
...
def sieve_subsequence(n_start, n_end, operators, k, x_max_factor=250, num_processes=None):
    """Sieve a chunk for a single subsequence (90n + k) using multiple processes."""
    if num_processes is None:
        num_processes = mp.cpu_count()
    
    chunk_size = (n_end - n_start) // num_processes
    if chunk_size == 0:
        return sieve_subsequence_chunk(n_start, n_end, operators, k, x_max_factor)
    ...
    with mp.Pool(processes=num_processes) as pool:
        results = pool.starmap(
            partial(sieve_subsequence_chunk, operators=operators, k=k, x_max_factor=x_max_factor),
            tasks
        )
    ...
    total_marks = 0
    raw_marks = 0
    for primes, t_marks, r_marks, _, _ in results:
    ...
    unique_density = total_marks / chunk_size if chunk_size > 0 else 0
    raw_density = raw_marks / chunk_size if chunk_size > 0 else 0
    return sorted(all_primes), total_marks, raw_marks, unique_density, raw_density

def sieve_chunk_chunk(n_start, n_end, operators, x_max_factor=250):
    """Sieve a chunk for A224854 (combined 90n + 11 and 90n + 13)."""
    chunk_size = n_end - n_start
    A = [0] * chunk_size
    marked_indices = set()
...
def sieve_chunk(n_start, n_end, operators, x_max_factor=250, num_processes=None):
    """Sieve for A224854 (combined 90n + 11 and 90n + 13) using multiple processes."""
    if num_processes is None:
        num_processes = mp.cpu_count()
    ...
    total_marks = 0
    raw_marks = 0
    for twin_primes, t_marks, r_marks, _, _ in results:
        all_twin_primes.extend(twin_primes)
        total_marks += t_marks
        raw_marks += r_marks
    ...
    unique_density = total_marks / chunk_size if chunk_size > 0 else 0
    raw_density = raw_marks / chunk_size if chunk_size > 0 else 0
    return sorted(all_twin_primes), total_marks, raw_marks, unique_density, raw_density

def sieve_a224854_density(h_values, num_processes=None):
    ...
    for h in h_values:
        ...
        primes_11, marks_11, raw_marks_11, unique_density_11, raw_density_11 = sieve_subsequence(
            n_start, n_end, operators_11, k=11, num_processes=num_processes
        )
        
        # Sieve for 90n + 13
        primes_13, marks_13, raw_marks_13, unique_density_13, raw_density_13 = sieve_subsequence(
            n_start, n_end, operators_13, k=13, num_processes=num_processes
        )
        
        # Sieve for A224854
        twin_primes, total_marks, raw_marks, unique_density, raw_density = sieve_chunk(
            n_start, n_end, all_operators, num_processes=num_processes
        )
