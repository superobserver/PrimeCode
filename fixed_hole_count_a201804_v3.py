...
def compute_overlaps_and_variance(h, silo_width=1000, validate_n=1000):
    """Compute operator overlaps, amplitude variance, and energy for A201804 with validation."""
    # Compute epoch
    n_max = 90 * h * h - 12 * h + 1
    ...
    for a, l, m, p, q, op_id in base_operators:
        ...
        while True:
            ...
            if new_p > max_prime or new_q > max_prime:
                break
            if is_prime(new_p) and is_prime(new_q) and (new_p * new_q) % 90 == 11:
    ...
    # Compute holes
    holes = [n for n in range(n_max + 1) if amplitude_map[n] == 0 and is_prime(90 * n + 11)]
    primes = [90 * n + 11 for n in holes]

    # Validation against original output for n <= validate_n
