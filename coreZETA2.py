...
def compute_reciprocal_zeta(args):
    k, n_max, s, operators = args
    holes = mark_composites(n_max, operators[k])
    zeta_k = sum((90 * n + k) ** (-s) for n in holes)
    reciprocal = 1 / zeta_k if zeta_k != 0 else 0
    return reciprocal, len(holes)
...
if __name__ == '__main__':
    print("Computing NORMALIZED SUM OF RECIPROCALS zeta for ALL 24 classes with multiprocessing for multiple s values")
    n_max = 10000000  # ~45M holes, fast test
    s_values = [
        0.5 + 14.134725j,  # First zero
        0.5 + 21.02204j,   # Second zero
        0.5 + 25.010858j,  # Third zero
        0.6 + 14.134725j,  # Off critical line
        0.5 + 15j          # On critical line, not a zero
    ]
    ...
    for s in s_values:
        ...
        with Pool(processes=num_cores) as pool:
            results = pool.map(compute_reciprocal_zeta, args)
        total_zeta = sum(result[0] for result in results)
        total_holes = sum(result[1] for result in results)
        normalized_total = total_zeta * (4 / 15)
        print(f"\ns = {s}")
        print(f"n_max={n_max}, Total Holes={total_holes}, Total sum of reciprocals={total_zeta}, Normalized sum={normalized_total}, |Normalized sum|={abs(normalized_total):.6f}")