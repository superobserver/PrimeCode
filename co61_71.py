...
print(primes)
end1 = time.monotonic()
print("Time elapsed during the process:", end1 - start1)
print("co61_71")
address = [i for i,x in enumerate(primes) if x == 0]
new_address = [(i+(start)) for i in address]
with open('61_71.csv', 'w', newline='') as file:
    # Step 4: Using csv.writer to write the list to the CSV file
    writer = csv.writer(file)
    writer.writerow(new_address) # Use writerow for single list

#close window
new_var = input("Your input here>") 
sys.exit()

###THIS PROGRAM CONTAINS THE "TIME ELAPSED DURING PROCESS" return and makes it a unicorn in terms of additional lines
