...
# Global variables
room_pairs = []  # List of (server_room, device_room) tuples
...
def setup():
    global room_pairs, nodes, edges, node_positions, buttons, nav_buttons, current_pair_index
    # Collect room pairs with incomplete cables
    room_pairs = sorted(set((cable.server_room, cable.device_room) for cable in manager.cables if cable.state != CableState.INSTALLED_TO_COMPLETION))
    if not room_pairs:
        room_pairs = [("", "")]  # Placeholder for empty state
        current_pair_index = 0

    # Reset current index if out of bounds
    if current_pair_index >= len(room_pairs):
        current_pair_index = 0

    # Update nodes and edges for the current pair
    nodes.clear()
    edges.clear()
    buttons.clear()
    if room_pairs[0][0]:  # Check if there are valid pairs
        server_room, device_room = room_pairs[current_pair_index]
        nodes[server_room] = "server"
        nodes[device_room] = "device"
        
        # Filter cables for the current pair
        for cable in manager.cables:
            if cable.server_room == server_room and cable.device_room == device_room and cable.state != CableState.INSTALLED_TO_COMPLETION:
                edges[(server_room, device_room)].append(cable)

        # Assign positions to nodes (centered for simplicity)
        node_positions[server_room] = (WIDTH * 0.3, HEIGHT * 0.4)
        node_positions[device_room] = (WIDTH * 0.3, HEIGHT * 0.6)

        # Create task buttons for the current pair
        deliverables = defaultdict(list)
        total_length = sum(cable.path_length_device or 0 for cable in edges[(server_room, device_room)])
        for cable in edges[(server_room, device_room)]:
        ...
        for deliverable, del_cables in deliverables.items():
            text = f"{deliverable}: {len(del_cables)} cables, {total_length:.1f} ft"
            button = {
    ...
    nav_buttons = [
        {"rect": pygame.Rect(WIDTH * 0.6, 50, 100, 40), "text": "Previous", "action": "prev"},
        {"rect": pygame.Rect(WIDTH * 0.75, 50, 100, 40), "text": "Next", "action": "next"}
    ]
...
def update_loop():
    ...
    # Display current room pair
    if room_pairs[0][0]:
        server_room, device_room = room_pairs[current_pair_index]
        title = f"{server_room} -> {device_room}"
    else:
        title = "No incomplete cables"
    title_text = font.render(title, True, BLACK)
    screen.blit(title_text, (WIDTH * 0.6, 20))
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            for button in nav_buttons:
                if button["rect"].collidepoint(mouse_pos):
                    if button["action"] == "prev" and current_pair_index > 0:
                        current_pair_index -= 1
                        setup()
                    elif button["action"] == "next" and current_pair_index < len(room_pairs) - 1:
                        current_pair_index += 1
                        setup()
            # Check task buttons
            for button in buttons:
