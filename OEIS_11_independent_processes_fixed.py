...
def process_operator(op_idx, l, m, z, o, limit, new_limit, result_queue):
    """Process a single operator, putting its marks array into the result queue."""
    try:
        marks_np = numpy.zeros(int(limit + 10), dtype=numpy.int32)
        for x in range(1, new_limit):
            drLD(x, l, m, z, o, marks_np, limit)
        result_queue.put((op_idx, marks_np))
    except Exception as e:
        print(f"Error in operator {op_idx + 1}: {e}")
        result_queue.put((op_idx, None))
...
def main():
    ...
    try:
        for op_idx, (l, m, z, o) in enumerate(operator_params):
            p = multiprocessing.Process(
                ...
                args=(op_idx, l, m, z, o, limit, new_limit, result_queue)
            )
            processes.append(p)
            p.start()
            if len([p for p in processes if p.is_alive()]) >= num_cores:
                for p in processes:
                    if p.is_alive():
                        p.join()
    ...
    while not result_queue.empty():
        op_idx, marks_np = result_queue.get()
        if marks_np is not None:
            marks_per_operator[op_idx] = marks_np
        else:
            print(f"Warning: Operator {op_idx + 1} failed to produce results")
            marks_per_operator[op_idx] = numpy.zeros(int(limit + 10), dtype=numpy.int32)

    # Merge results
    A201804 = numpy.zeros(int(limit + 10), dtype=numpy.int32)
    for marks in marks_per_operator:
        A201804 += marks
