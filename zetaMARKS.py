...
def compute_holes_and_marks(n_max, amplitude, coprime_24):
    total_holes = {k: 0 for k in coprime_24}
    total_marks = {k: 0 for k in coprime_24}
    seen_primes = set()
    for k in coprime_24:
        for n in range(n_max + 1):
            p = 90 * n + 1 if k == 1 else 90 * n + k
            if p <= 2.15e9:
                if amplitude[k][n] == 0 and p not in seen_primes:
                    total_holes[k] += 1
                    seen_primes.add(p)
                elif amplitude[k][n] > 0:
                    total_marks[k] += amplitude[k][n]  # Sum all hits
    return total_holes, total_marks
...
if __name__ == '__main__':
    print("Computing Total Marks and Holes per Class")
    n_max_small = 1000  # Small run for first 15
    n_max_full = 23900001  # Full run for analysis
