...
# Play MP3 using Pygame with space bar skip
                ...
                while pygame.time.get_ticks() - start_time < duration:
                    for event in pygame.event.get():
                        if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE:
                            sound.stop()
                            logging.info(f"Audio playback skipped for {audio_file}")
                            print(f"Audio playback skipped for {audio_file}")
...
def animate_holes(coprime_24, prime_counts):
    if 'pygame' not in missing_modules:
        ...
        pygame.display.set_caption("Prime Distribution for k=11")
        WHITE = (255, 255, 255)
        BLACK = (0, 0, 0)
        BLUE = (0, 0, 255)
        ...
        while running:
            ...
            label_title = font.render("Prime Distribution for k=11", True, BLACK)
    ...
    if 'pygame' not in missing_modules:
        ...
        for op_idx, cancels in enumerate(cancellations):
            if cancels:
                ...
                p_x = p if p != 0 else 0
                point_x = 50 + n * x_scale
                point_y = HEIGHT - 50 - operator_amplitude[n] * y_scale
                points.append((point_x, point_y, COLORS[op_idx % len(COLORS)]))
                vectors.append((50, HEIGHT-50, point_x, point_y, COLORS[op_idx % len(COLORS)]))  # Quadratic vector
                if p_x > 0:
                    # Map p_x to angle: smaller p_x -> flatter (closer to 0), larger p_x -> steeper
                    angle = math.pi / 4 * (100 / max(p_x, 1))  # Inverse mapping, capped at pi/4
                    vector_length = 50  # Fixed length for visibility
                    end_x = point_x + vector_length * math.cos(angle) * x_scale
                    end_y = point_y - vector_length * math.sin(angle) * y_scale
                    vectors.append((point_x, point_y, end_x, end_y, COLORS[op_idx % len(COLORS)]))  # Frequency vector
...
def generate_thesis_presentation(amplitude, last_holes, operator_amplitude, operator_amplitudes, operator_count, operator_cancellations, operators_k11):
    ...
    presentation = [
        ("Esteemed References, I present my thesis: a deterministic quadratic sieve for prime identification in residue classes modulo 90. This work deconstructs base-10 integers into algebraic observables—digital roots, last digits, and amplitudes—for residue class k=11. Unlike eliminative sieves, my approach constructs composites using quadratic operators, leaving primes as unmapped residuals, revealing deep symmetries in the number-theoretic map space.", 'male', None, None),
        ("The Liouville function, defined as lambda of n equals (-1) to the power Omega of n, where Omega is the number of prime factors, indicates that neighbors on the number line are not chained. This enables our sieve to de-interlace residue class modulo 90. The class 90n+11 captures primes >=7. The quadratic operators act as composite frequency generators, producing addresses in a symmetrical distribution.", 'male', (lambda screen: screen.blit(pygame.font.SysFont('arial', 30).render("λ(n) = (-1)^Ω(n)", True, (0, 0, 0)), (WIDTH//2-100, HEIGHT//2)),), "Liouville Function Popup"),
        ("I, Eratosthenes, argue my sieve marks multiples sequentially on the number line, efficiently identifying primes. Why de-interlace into residue classes when a linear approach suffices?", 'female', None, None),
        ("With respect, Eratosthenes, your sieve scales as O(n log log n) and lacks algebraic insight. Our sieve’s residue class for k=11 forms a map space where composites are generated quadratically, achieving near O(1) complexity with parallelized operators. This de-interlacing, supported by the Liouville function’s non-chaining, reveals symmetries that underpin the prime distribution.", 'male', None, None),
        ("The modulus 90, the least common multiple of 2, 3, and 5, filters trivial primes. The Euler totient function, phi of n equals n times the product over prime p dividing n of (1 minus 1 over p), yields phi(90)=24, defining residues coprime to 90. The class k=11, einselected to limit variance, partitions numbers with digital roots in {1,2,4,5,7,8} and last digits in {1,3,7,9}. Measuring this class confidently predicts the prime distribution.", 'male', (lambda screen: screen.blit(pygame.font.SysFont('arial', 30).render("φ(n) = n ∏_{p|n} (1 - 1/p)", True, (0, 0, 0)), (WIDTH//2-150, HEIGHT//2)),), "Euler Totient Function Popup"),
        ("I, Euler, question the fragmentation of the number line into residue classes. My zeta function unifies primes globally. Does your partitioning preserve this unity?", 'female', None, None),
        ("Euler, your zeta function guides us. The k=11 class de-interlaces the number line, exposing local symmetries that complement your global view. Its zeta function converges to zeros on Re(s)=1/2, aligning with the Riemann Hypothesis. The einselection of this class ensures finite variance, enabling precise prime measurement.", 'male', None, None),
        ("Our sieve’s completeness is algebraic. For residue class k=11, 12 quadratic operators, n=90x^2-lx+m, generate all composites 90n+11. For n=4, this yields 90*4+11=371=7*53. Parallel processing marks multiples, ensuring no composite escapes. Primes remain as holes with amplitude zero.", 'male', None, None),
        ("I, Pomerance, note my quadratic sieve factorizes efficiently. Could your finite operators miss composites?", 'female', None, None),
        ("Pomerance, your sieve inspires us. For any composite 90n+11=a*b, an operator captures n via n=m+pt+qs+90st. With 12 operators, all factorizations are covered. The distribution of primes for k=11 is visualized here, showcasing the map space’s structure.", 'male', (animate_holes, coprime_24, prime_counts), "Prime Distribution for k=11"),
        ...
        ("To further illustrate, we represent each operator’s first quadratic address as a dot, with vectors indicating the quadratic value and cancellation frequency. The quadratic vector points to the address, and the frequency vector’s flatness reflects the cancellation rate, with smaller frequencies like 7 being nearly horizontal and larger ones like 97 steeper, forming arrows on a 2D grid.", 'male', (animate_operator_vectors, operator_cancellations, operator_amplitude, operators_k11), "Operator Vector Dynamics for k=11"),
        ("The Riemann Hypothesis posits that non-trivial zeros of the zeta function lie on Re(s)=1/2. Our class-specific zeta function for k=11 encodes the prime sequence, with symmetries mandating zero convergence. The amplitude dynamics for k=11, shown here, illustrate how primes emerge as holes amidst quadratic cancellations.", 'male', (animate_amplitude, n_values, amplitudes_k11), "Amplitude Dynamics for k=11"),
        ("In conclusion, our quadratic sieve for k=11 partitions primes >=7, leveraging de-interlacing and quadratic operators to reveal symmetrical composite distributions. Its completeness, validated algebraically, and support for the Riemann Hypothesis provide a foundation for future exploration.", 'male', None, None),
    ...
    for i, (text, voice_gender, animation_data, animation_title) in enumerate(presentation):
        try:
            ...
            if generate_tts_mp3(text, filename, voice_gender):
                if not play_tts_mp3(filename):  # Returns False if skipped
                    logging.info(f"Skipped to next section: {i+1}")
                    print(f"Skipped to next section: {i+1}")
            else:
