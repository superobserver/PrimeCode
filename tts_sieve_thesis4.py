...
def write_animation_scripts():
    """
    Write external Pygame animation scripts (anim_91.py, anim_92.py) to tts_output.
    """
    scripts = [
        {
            "filename": "anim_91.py",
            "title": "Prime Counts Animation for k=11, k=17",
            "code": """
import pygame
import numpy as np
...
"""
        ...
        {
            "filename": "anim_92.py",
            "title": "Zeta Function Convergence Animation for k=11",
            "code": """
import pygame
import numpy as np
...
def plot_process(plot_script_filename, title):
    """
    Launch an external Pygame animation script using subprocess.Popen.
    """
    if 'pygame' not in missing_modules:
...
def generate_thesis_presentation():
    ...
    for text, gender in completeness:
        presentation.append((text, gender, "anim_91.py" if text == completeness[-1][0] else None, "Prime Counts Animation for k=11, k=17" if text == completeness[-1][0] else None))
    ...
    riemann = [
        ...
        ("Riemann, your insight guides me. My 24-class partition deinterlaces the number line, revealing local symmetries that enforce zero convergence. The holes—primes like 743 for k=11—form an infinite sequence entangled with the operator algebra, mandating Re(s)=1/2. Behold the zeta convergence animation to see this alignment.", 'male')
    ]
    for text, gender in riemann:
        presentation.append((text, gender, "anim_92.py" if text == riemann[-1][0] else None, "Zeta Function Convergence Animation for k=11" if text == riemann[-1][0] else None))
